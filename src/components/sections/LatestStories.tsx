...
export default function LatestStories() {
  ...
  return (
    <section id="latest" className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 min-w-0">
      {/* Header Row */}
      <SectionHeader
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div
            ref={scrollRef}
            className="w-full flex lg:hidden gap-6 overflow-x-auto no-scrollbar snap-rail -mx-4 px-4 py-2 min-w-0"
          >
            {latestItems.map((article) => (
              <div
                key={article.id}
                className="w-[248px] h-[285px] sm:w-[48%] sm:h-auto md:w-[31%] flex-shrink-0 snap-item"
              >
