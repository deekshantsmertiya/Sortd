    ...
    <section className="relative w-full bg-sortd-off-white overflow-hidden border-b border-gray-200 flex flex-col">
      ...
      {priorityArticle && (
        <Link href={`/article/${priorityArticle.slug}`} className="xl:hidden block w-full">
          <motion.div
            className="relative w-full h-[238px] sm:h-[280px] overflow-hidden border-t border-b border-gray-200 group"
          >
            {/* Background Image */}
            <Image
              src={priorityArticle.imageUrl}
            ...
            <div className="absolute inset-0 flex flex-col justify-center items-start pl-5 pr-5 gap-0">
              {/* Yellow Title Block */}
              <motion.div
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
