  ...
  return (
    <Link href={`/article/${article.slug}`} className={`group flex flex-col h-full w-full ${className}`}>
      ...
      >
        ...
        <div className="h-[130px] pt-[12px] px-[12px] pb-[14px] flex flex-col justify-between flex-shrink-0 bg-white">
          <div className="flex flex-col">
            {/* Category Tag (Inline, below image) */}
            <div
              className={`self-start h-[22.44px] min-w-[70px] px-[8.97px] gap-[8.97px] rounded-none font-body font-bold text-[9px] uppercase tracking-wider inline-flex items-center justify-center select-none mb-[8px] ${badgeStyle.textColorClass}`}
            ...
            {/* Uppercase Headline */}
            <h3 className="font-heading font-normal text-[20px] leading-[21px] tracking-[0.4px] uppercase text-[#0A0A0A] line-clamp-1 group-hover:text-sortd-pink transition-colors mb-[6px]">
              {article.title}
            </h3>
