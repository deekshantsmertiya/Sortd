  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-1.5 px-4 py-2 rounded-[10px] border-2 border-sortd-yellow bg-transparent hover:bg-sortd-yellow/5 text-xs font-bold text-sortd-yellow transition-all"
        aria-expanded={isOpen}
      >
        <MapPin className="w-4 h-4 text-sortd-yellow" />
        <span>{selected.name}</span>
        <ChevronDown className={`w-3.5 h-3.5 text-sortd-yellow transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`} />
      </button>
