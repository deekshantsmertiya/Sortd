...
export default function AdminPage() {
  ...
  const handleSaveVideo = (e: React.FormEvent) => {
    e.preventDefault();
    if (!videoTitle || !videoThumbnailUrl || !videoDescription) {
      alert("Please fill in all required fields.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#F7F9FC] flex flex-col lg:flex-row text-sortd-black">
      
      {/* Sidebar for Desktop */}
      <aside className="hidden lg:flex flex-col justify-between w-64 bg-slate-900 text-white min-h-screen p-6 select-none shrink-0 border-r border-slate-800">
        <div>
          {/* Logo Brand */}
          <div className="relative w-[79.5px] h-[111px] mb-8 mt-2 mx-auto">
            <Image
              src="/Footer.png"
              alt="Sortd Logo"
              fill
          ...
          <h1 className="font-heading font-normal text-xl tracking-wider uppercase text-center mb-10 border-b border-white/5 pb-4">
            Dashboard
          </h1>
        ...
        {/* Sidebar Footer */}
        <div className="flex flex-col gap-3 font-body font-bold text-xs tracking-widest uppercase border-t border-white/5 pt-4">
          <Link
            href="/"
                    ...
                    {filteredUsers.map((u) => {
                      ...
                      return (
                        <tr key={u.username} className="hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="py-4 px-6 text-right">
                            {!isLockedUser ? (
                              <button onClick={() => handleDeleteUser(u.username)} className="p-1.5 rounded-[6px] hover:bg-red-50 text-red-600 transition-colors" title="Delete User"><Trash2 className="w-4 h-4" /></button>
                            ) : (
              ...
              <form onSubmit={handleCreateUser} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    required
                    placeholder="e.g. editor_dubai"
                    value={newUsername}
                    onChange={(e) => setNewUsername(e.target.value)}
                    className="w-full px-4 py-3 bg-gray-55 border border-gray-200 focus:border-sortd-black rounded-[8px] text-xs font-semibold focus:outline-none"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 bg-gray-55 border border-gray-200 focus:border-sortd-black rounded-[8px] text-xs font-bold text-sortd-grey uppercase tracking-wider bg-white focus:outline-none"
                  >
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-fade-in">
            ...
            <form onSubmit={handleSavePost} className="flex-grow overflow-y-auto p-6 flex flex-col gap-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label className="block text-[10px] font-bold text-sortd-grey uppercase tracking-wider mb-1.5 font-body">Image URL (Required)</label>
                  <input type="url" required placeholder="https://images.unsplash.com/..." value={postImageUrl} onChange={(e) => setPostImageUrl(e.target.value)} className="w-full px-4 py-3 bg-gray-55 border border-gray-200 focus:border-sortd-black rounded-[8px] text-xs font-medium text-sortd-black placeholder:text-gray-400 focus:outline-none transition-colors" />
                </div>
                <div>
          ...
          <div className="bg-white w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-fade-in">
            ...
            <form onSubmit={handleSaveVideo} className="flex-grow overflow-y-auto p-6 flex flex-col gap-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-sortd-grey uppercase tracking-wider mb-1.5 font-body">Duration (e.g. 02:15)</label>
                  <input type="text" required placeholder="Duration tag..." value={videoDuration} onChange={(e) => setVideoDuration(e.target.value)} className="w-full px-4 py-3 bg-gray-55 border border-gray-200 focus:border-sortd-black rounded-[8px] text-xs font-semibold text-sortd-black focus:outline-none transition-colors" />
                </div>
              </div>
              <div>
                <label className="block text-[10px] font-bold text-sortd-grey uppercase tracking-wider mb-1.5 font-body">Thumbnail Image URL (Required)</label>
                <input type="url" required placeholder="https://images.unsplash.com/..." value={videoThumbnailUrl} onChange={(e) => setVideoThumbnailUrl(e.target.value)} className="w-full px-4 py-3 bg-gray-55 border border-gray-200 focus:border-sortd-black rounded-[8px] text-xs font-medium text-sortd-black focus:outline-none transition-colors" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-[10px] font-bold text-sortd-grey uppercase tracking-wider mb-1.5 font-body">Main Description (Optional)</label>
                <textarea rows={4} placeholder="Full contextual detail description to show inside the player modal..." value={videoLongDescription} onChange={(e) => setVideoLongDescription(e.target.value)} className="w-full px-4 py-3 bg-gray-55 border border-gray-200 focus:border-sortd-black rounded-[8px] text-xs font-medium text-sortd-black focus:outline-none resize-y" />
              </div>
              <div className="flex items-center justify-end gap-3 mt-4 pt-4 border-t border-gray-100 select-none">
